...
from upgrade_agent.state import AgentState, AvailableUpdate, UpdateAttempt, UpdateStatus, TraceEvent, Dependency
...
from upgrade_agent.prompts.reason import build_reasoning_prompt, build_planning_prompt, build_error_analysis_prompt

from upgrade_agent.tools.dependencies import get_all_dependencies, check_pypi_version, check_dockerhub_version
from upgrade_agent.tools.github import github_create_branch, github_update_file, github_create_pr
from upgrade_agent.tools.execution import run_tests
from upgrade_agent.tools.memory import read_memory, write_memory, append_memory
...
def observe(state: AgentState) -> AgentState:
    ...
    add_trace(state, "observe_complete", "observe", {
...
def reason(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            state["decision"] = {"decision": "PROCEED_WITH_CAUTION", "reasoning": response.content}
    except Exception as e:
        state["reasoning"] = f"Error: {str(e)}"
    
    add_trace(state, "reason_complete", "reason", {
        "update": update.dict(),
...
def plan(state: AgentState) -> AgentState:
    ...
    add_trace(state, "plan_complete", "plan", {
...
def act(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if old_version and old_version != "latest":
            new_content = content.replace(f"{name}=={old_version}", f"{name}=={new_version}")
        else:
        ...
        if branch_result.get("success"):
            # Commit file
            commit_result = json.loads(github_update_file.invoke(
                path=file_path,
                content=new_content,
    ...
    add_trace(state, "act_complete", "act", {
...
def fix(state: AgentState) -> AgentState:
    ...
    add_trace(state, "fix_complete", "fix", {
...
def reflect(state: AgentState) -> AgentState:
    ...
    add_trace(state, "reflect_complete", "reflect", {
