...
def extract_stage_summary(stage_name, raw_output):
    ...
    if stage_name.lower() == "update":
        ...
        if pkg_matches and not upgrade_match:
            summaries.append(
                f"{len(pkg_matches)} package{'s' if len(pkg_matches) != 1 else ''} upgraded"
            )
...
def parse_brew_events(log_lines=None):
    ...
    for line in log_lines:
            ...
            if current_event:
                ...
                if current_stages:
                    ...
                    current_event["status"] = (
                        "failed"
                        if any(s["status"] == "failed" for s in current_event["stages"])
                        else "warning"
                        if any(
                            s["status"] == "warning" for s in current_event["stages"]
                        )
                        else "success"
                    )
                events.append(current_event)
        ...
        # Check for stage markers
            ...
            stage_match = re.search(
                r"STAGE:(\w+):(started|success|failed|warning)", line
            )
            if stage_match:
        ...
        # Check for event end marker
            ...
            if end_match:
                ...
                for stage_name, stage_data in current_stages.items():
                    ...
                    stage_data["summary"] = extract_stage_summary(
                        stage_name, stage_data["raw_output"]
                    )
                ...
                upgrades = sum(
                    1 for s in current_event["stages"] if s["name"].lower() == "upgrade"
                )
                if upgrades > 0:
    ...
    if not events:
        ...
        for group in event_groups:
            ...
            status = "success"
            if any(
                pattern in full_content.lower()
                for pattern in ["error", "failed", "command not found"]
            ):
                status = "failed"
            elif any(
                pattern in full_content.lower() for pattern in ["warning", "deprecated"]
            ):
                status = "warning"
            ...
            event = {
                ...
                "duration_seconds": int(
                    (event_end_time - event_start_time).total_seconds()
                ),
                "stages": stages,
                "overall_summary": stages[1]["summary"]
                if len(stages) > 1
                else "No changes",
                "raw_logs": group,
            }
...
def generate_demo_logs():
    """
    Generate realistic demo log entries.
    ...
    demo_entries.append(
        {
            ...
            "message": "[EVENT] END:brew:success - Brew maintenance finished successfully",
            "level": "success",
        }
    )
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=4)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "==> Upgrading 2 outdated packages: libiconv, opencode",
            "level": "info",
        }
    )
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=4)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "==> Removing old versions. Freed 248.3MB.",
            "level": "info",
        }
    )
    ...
    demo_entries.append(
        {
            ...
            "message": "Warning: Deprecated formula. Please migrate to node@18.",
            "level": "warning",
        }
    )
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=48)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "Warning: python 3.10.9 is deprecated",
            "level": "warning",
        }
    )
    ...
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=72)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "Error: Network timeout after 30 seconds. Failed to download bottles.",
            "level": "error",
        }
    )
    ...
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=24)).strftime("%Y-%m-%d %H:%M:%S"),
    ...
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=24)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "Updated 1 tap (homebrew/core).",
            "level": "info",
        }
    )
    demo_entries.append(
        {
            "timestamp": (now - timedelta(hours=96)).strftime("%Y-%m-%d %H:%M:%S"),
            "message": "Your system is up-to-date. No upgrades available.",
            "level": "info",
        }
    )
...
def get_logs():
    ...
    if view == "events":
        ...
        # Calculate summary
        all_events = (
            parse_brew_events() if os.path.exists(LOG_FILE) else generate_demo_events()
        )
        summary = {
    ...
    # VIEW: LOGS (old format - for backward compatibility)
        ...
        if os.path.exists(LOG_FILE) and not demo_mode:
            ...
            for line in lines:
                ...
                log_entries.append(
                    {"timestamp": timestamp, "message": line, "level": level}
                )
